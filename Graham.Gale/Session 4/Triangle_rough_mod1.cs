...
namespace Triangle
{
     ...
     */
       ...
       //Here is the 'basic math' to determine if it's a triangle:
       //            if ( a + b > c
       //            and  a + c > b
       //            and  b + c > a) then it's a triangle. 
       //Use 1, 2, 3 is NOT a triangle; 2, 3, 4 IS a triangle.
    ...
    public class TriangleTester
    {
        ...
        public static TriangleType GetTriangleType(int a, int b, int c)
        {
            ...
            //Here's the code to validate that the variables are integers.
            //Need to check if decimal numbers pass this check, if so build code to check that it's a whole number. 
            int[] values = new int[3] {a, b, c};
	          bool res = int.TryParse(a, out numA);
	          bool res = int.TryParse(b, out numB);
	          bool res = int.TryParse(c, out numC);
	          	          if (res == false)
	            {
	          ...
	          //Need to do int.maxvalue after validating that variables are numbers greater than 0 
        	  // ? Something like:
        	  // if a int.maxvalue and b = int.maxvalue and 
        	  //    c.int.maxvalue the it's an Equilateral
        	  // if a int.maxvalue and b = int.maxvalue and 
        	  //    c < .int.maxvalue the it's an Isosceles
        	  // if one side is int.maxvalue and the sum of the other two is > int.maxvalue
                ...
                //and  b + c > a)
            {
                    ...
                    //There are only two distinct values in the set, therefore two sides are equal and one is not
                {
                    ...
                    // There are three distinct values in the set, therefore no sides are equal
                {
